pipeline {
	...
	//	  sh 'docker-compose up -d'
                ...
                sh 'docker run -v "$(pwd)/var/myapi:/usr/src/app/acebook/api"  -p "4000:4000" -d  jacksemis101/ace-book-api'
            }
        }
 	  stage('Build and Run docker Frontend') {
            steps {  
		        ...
		        sh 'docker run -v "$(pwd)/var/myfront:/usr/src/app/acebook/api"  -p "3000:3000" -d  jacksemis101/ace-book-front'
